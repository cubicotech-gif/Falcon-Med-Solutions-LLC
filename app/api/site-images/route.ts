...
const EXPECTED_KEYS = new Set(EXPECTED_SLOTS.map((s) => s.slot_key))

async function syncSlots(supabase: ReturnType<typeof getSupabaseAdmin>) {
  ...
  // Delete any slots that are NOT in our expected list (old/stale keys)
  const staleKeys = (existing || [])
    .map((r: { slot_key: string }) => r.slot_key)
    .filter((key: string) => !EXPECTED_KEYS.has(key))
...
export async function GET() {
  try {
    ...
    // Sync: add missing slots + remove stale old ones
    await syncSlots(supabase)
